...
use super::{
	mock::{
		assert_events, new_test_ext, Balances, Bridge, ChainId, Call, Event, Origin,
...
use webb_resourceid::{compute_chain_info, derive_resource_id};
	...
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Bridge::whitelist_chain(
				Origin::root(),
				compute_chain_info(ChainId::get(), SUBSTRATE_CHAIN_TYPE)
			),
			Error::<Test>::InvalidChainId
		);

		assert_events(vec![Event::Bridge(pallet_bridge::Event::ChainWhitelisted { chain_info: 0 })]);
	})
}
...
fn create_sucessful_proposal() {
	let src_info = compute_chain_info(1u32, [2u8, 0u8]);
	let r_id = derive_resource_id(src_info, b"remark");

	new_test_ext_initialized(src_info, r_id, b"System.remark".to_vec()).execute_with(|| {
		let prop_id = 1;
		...
		let prop = Bridge::votes(src_info, (prop_id, proposal.clone())).unwrap();
		...
		let prop = Bridge::votes(src_info, (prop_id, proposal)).unwrap();
		let expected = ProposalVotes {
...
fn create_unsucessful_proposal() {
	let src_info = compute_chain_info(1u32, [2u8, 0u8]);
	let r_id = derive_resource_id(src_info, b"transfer");
